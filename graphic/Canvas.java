...
/***
 * canvas拥有绘制调用，为了绘制，你需要四个基本组件：一个存储像素的位图，
 * 一个拥有绘制能力的canvas，绘制内容和画笔（画什么，怎么画）
 */
public class Canvas {
    ...
    */
    ...
    /***
     * 用一个位图构建一个canvas，向该bitmap写入内容，该位图必须是可变的
     * canvas的初始化密度和这个位图的密度相同
     */
    ...
    /***
     * 保存当前矩阵和剪切到一个私有栈，随后的缩放，旋转，位移，调用，倾斜，合并等操作
     * 都不变，但是当restore（）调用时，这些操作会被忽略
     */
