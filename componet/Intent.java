 ...
 */
//牵涉到数据在不同进程间的传递，因此需要实现parcelable接口
//一个intent对象，代表了什么，用途是什么等
//系统会对intent对象进行解析并找到相应的启动对象，如何解析，解析过程在哪
public class Intent implements Parcelable, Cloneable {
     ...
     * by the system.
     */
    //这个是电池服务发送的广播的action，sendBroadcast(intent);
    @SdkConstant(SdkConstantType.BROADCAST_INTENT_ACTION)
   ...
   /***
    ...
    // ---------------------------------------------------------------------
    //定义与intent相关的数据，包括action data package flag category extra sourcebound
    //系统根据这些信息解析该intent并启动响应的组件进行处理
     ...
     * @see #toUri
     */
    //通过uri和flag进行intent解析
    //uri必须以intent:为schema
    ...
    /***
     * 获取activity组件，该activity用来处理这个intent，根据intent信息判读
     ...
     */
    //从xml文件中解析出intent对象
    //属性不只是界面上的啊，什么都可以，我太局限了
    //大量使用pull解析方式，各种啊，还会进行其子元素的解析，比如category等
