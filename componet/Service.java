...
/***
 *把不需要与用户交互的部分用service来实现，注意：service和thread没有丝毫的关系，完全是两个
 *不同范畴的概念，service和activity是同级概念，而thread和process是同级概念，thread是程序的
 *一个执行单元，而service是android的一种组件机制
 ...
 *      bind}
 */
//相对activity来说简单许多，因为不牵涉到和用户交互及界面的构建
//继承于contextwrapper，实现了compentcallback接口，和activity进行比较，没有与界面相关的操作
//现在看来这些真是没什么技术含量
//componentcallback接口
//android为我们提供了实现service的简便的客户端实现
//但是我想知道的是其如何实现
public abstract class Service extends ContextWrapper implements ComponentCallbacks2 {
    ...
    /***
     * 该服务第一次创建时由系统调用，不能直接调用,其实什么也没做，所以覆盖时没必要super.oncreate
     */
    public void onCreate() {
    }

    /***
     * 已经过时，请使用onstartcommand代替
    ...
    /***
     * 当每次一个客户端通过startservice明确调用时由系统调用该方法，提供参数。。。。
     ...
     */
    //默认实现返回的值为
    //传递的参数intent（startservice（intent）） flag startid
    ...
    /***
     * 返回一个链接到该服务的通道。返回的ibinder对象是一个用aidl描述的接口
     ...
     * <a href="{@docRoot}guide/topics/fundamentals/processes-and-threads.html">Processes and
     * Threads</a>.</p>
     * intent  用来绑定到该服务的intent
     * 返回一个客户端能够在该服务上调用的ibinder对象
