...
/***
 * 一个activity代表了一个用户能够交互的单独的事情，因为要与用户交互，activity
 * 一般会创建一个window，你可以在这个window里使用setcontentview放置你的ui控件
 * 一般来说，activity一般会表现为一个全屏的窗口，但是也可以用其他方式呈现，比如浮动
 * 窗口（通过为其设置android.R.attr#windowIsFloating主题），或者嵌入其他activity（activitygroup)
 * 对于任何一个activity子类来说，一般要实现oncreate和onpause方法 
 * 为了使用context的startactivity等方法，一个activity需要在main文件中正确的注册
 ...
 * </ol>
 * 
 * 有时候一个activity需要一个长时间的操作，比如拍摄了一张照片后上传，该上传可以放到一个
 * 服务里，这允许用户切换到其他activity，而不会影响上传
 */
//继承自contextthemewrapper，和dialog比较，dialog是创建了一个contextthemewrapper的实例，而activity本身就是
//需要什么样的初始化呢
public class Activity extends ContextThemeWrapper
    ...
    //一个activity是需要一个window的
    private Window mWindow;
    //同样必须有一个对应的windowmanager
    ...
    //获取windowmanager以显示自定义窗口
    public WindowManager getWindowManager() {
        return mWindowManager;
    }

    //返回该activity持有的window
    ...
    /***
     * 会被系统在创建该activity时调用，参数的含义自己体会吧
     ...
     */
    //final类型，不能改变，保证activity的正常运行，会调用onRestoreInstanceState
    //为我们提供一个机会
     ...
     * @see #onPause
     */
    //比较重要的一个方法，当系统要回收该activity时给我们一个保留其状态的机会，
    //注意其不是activity生命周期里的概念,看其默认实现
    ...
    /***
     * 是keydown的最终归宿，即其包含的所有view都未对其进行处理的情况下
     ...
     */
    //在该activity管理的windowmanager被添加到windowmanager时调用
     ...
     * @see #startActivity 
     */
    //参数多出了一个requestcode，即返回结果代码，该方法会被谁调用？被我们自己调用
     ...
     *         reply, or null if none.
     */
    //返回调用该activity的activity，该activity是setResult的对象
     ...
     */
    //获取所处task的id，该任务由activitymanager完成
    ...
    //根据名字获取系统服务，直接调用context的方法，从这可以看错，activity其实是一个功能集成
    @Override
    public Object getSystemService(String name) {
        if (getBaseContext() == null) {
            throw new IllegalStateException(
            		//oncreate之前是不能获取服务的，为什么，看oncreate的实现吧
                    "System services not available to Activities before onCreate()");
        }
        //本身实例化了两个服务，直接返回，可见其性能考虑的周全
     ...
     */
    //终于明白为什么要实例化一个handler了，就这用了一丢丢啊
    ...
    //牛逼的attach方法，activity的很多赋值都在attach方法里进行，包括窗口创建、窗口回调设置、线程设置
    //windowmanager的设置等
    ...
    //activity token是什么，作用又是什么
    final IBinder getActivityToken() {
        return mParent != null ? mParent.getActivityToken() : mToken;
    }
    //这是activity创建的方法，会被调用，调用的时机等是掌握activity生命周期的关键
    ...
    //执行resume，final方法，不能覆盖或者重写，谁来调用
    final void performResume() {
    	//竟然调用了performrestart，是为了兼容
        performRestart();
        ...
        if (!mCalled) {
            throw new SuperNotCalledException(
            	//说明没有调用onPostResume方法
                "Activity " + mComponent.toShortString() +
                " did not call through to super.onPostResume()");
        }
    }
    //onPause是我们有能力在系统处理pause时执行我们的操作，真正的操作在performpause实现
    //后天只是决定activity何时pause等，具体实现还是activity自身实现
    //马丹，原来这么简单
    ...
    final void performStop() {
        ...
        if (!mStopped) {
            ...
            mCalled = false;
            //操蛋，知道instrumentation为什么能监视activity了
    ...
    //这个方法是做什么用,会被谁进行调用等等，分发结果并调用onActivityResult方法
    void dispatchActivityResult(String who, int requestCode, 
        ...
        mFragments.noteStateNotSaved();
        if (who == null) {
        	//调用了onactivityresult方法,供我们定义自己的行为
