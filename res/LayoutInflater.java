...
/***
 * 该类用来实例化布局文件到对应的视图对象，一般不会直接实例化该对象，而是通过activity
 * 的getLayoutInflater来获得一个已经配置完成的绑定了context的实例,抽象类
 */
//开该类如何实现导入view树
public abstract class LayoutInflater {
     ...
     *         the inflated XML file.
     */
    //看看其是如何实现的吧，以后就不会对这有什么疑惑了
    public View inflate(XmlPullParser parser, ViewGroup root, boolean attachToRoot) {
        synchronized (mConstructorArgs) {
            final AttributeSet attrs = Xml.asAttributeSet(parser);
            mConstructorArgs[0] = mContext;
            //直接将传递的root作为根视图，最终是root视图
            View result = root;
    ...
    //通过名字实例化view的底层方法
    //这些根据名字得到其对应的类
    //几个参数===》name将要导入的视图的全称，attrs为该view的属性,prefix为前缀===》比如android.view
    public final View createView(String name, String prefix, AttributeSet attrs)
        ...
        try {
            ...
            args[1] = attrs;
            //实例化该view即可,如何进行处理的，如何得到该view class的
