...
/***
 * 视图结构的顶层，实现了视图和windowmanager之间必要的协议，windowmanagerimpl
 * 是它的一种大部分实现？？？
 * 这是handler的一个子类，说明她实现的是？？？
...
//其可以处理消息，持有它的引用便可以发送消息和处理消息，实现了viewparent接口
public final class ViewRoot extends Handler implements ViewParent,
     ...
     */
    //音频管理，难道是实现playsoundeffect？
    AudioManager mAudioManager;
    //密度信息
    private final int mDensity;
    //主消息循环的和窗口进行对话的windowsession？？完全不懂啊
    ...
    */
    ...
    /***
     * 拥有一个孩子，setView方法所设置的这个view是view树的rootview
     */
    //该方法会在windowmanager的addview方法中被调用
    //setview方法的作用是什么，会在何时何处被调用
    public void setView(View view, WindowManager.LayoutParams attrs,View panelParentView) {
        synchronized (this) {
            ...
            if (mView == null) {
                ...
                //绑定信息，其rootview是该view??或者说其就是rootview？
     ...
     * {@inheritDoc}
     */
    //在此处啊  scheduleTraversals()方法被频繁调用，说明很重要
    ...
    //该即为顶级视图，无父view,真的是这样么，这个吧，viewroot本来就不是view，呵呵呵呵呵呵呵
    ...
    //该方法被频繁调用，会发送一个do_traversal消息，回去看一下该消息的处理
    public void scheduleTraversals() {
        if (!mTraversalScheduled) {
            mTraversalScheduled = true;
            //该消息会被在handlemesseng里进行处理
    ...
    //hostview即mView，首先保证mappvisible可见，然后在判断mvie的可见性
    int getHostVisibility() {
        return mAppVisible ? mView.getVisibility() : View.GONE;
    }

    //该类为私有，不能随意更改，供内部使用，该方法频繁调用，进行view树的绘制等，是scheduleTraversals的处理函数？？
    private void performTraversals() {
        // 绘制view树的起点，很多处理view的方法
        //这里的host代表什么，就是mview啊，如此这般
        ...
        if (mFirst) {
            ...
            mLayoutRequested = true;
            //获取屏幕的宽和高，这是整个窗口的大小确定
            DisplayMetrics packageMetrics =
                mView.getContext().getResources().getDisplayMetrics();
            //好暴力啊，直接用
            desiredWindowWidth = packageMetrics.widthPixels;
            desiredWindowHeight = packageMetrics.heightPixels;
            //在第一次，告诉view树其被依附到了窗口
        ...
        if (mFirst || windowShouldResize || insetsChanged
            ...
            if (focusChangedDueToTouchMode || mWidth != host.mMeasuredWidth
                ...
                //一个递归的过程，从顶至下，需要理解这种递归的思想并能灵活运用
                host.measure(childWidthMeasureSpec, childHeightMeasureSpec);
        ...
        if (didLayout) {
            ...
            //host进行布局，布局参数为（0，0，width,height）
            //直接调用host的放置方法，进行一个递归的layout过程，该方法会一直传递的
    ...
    /***
     * 根据rootview在窗口中的布局参数指出为其提供的measurespec
    ...
    //绘制的开始，fullredrawneeded参数的含义,是否全部重新绘制，这个直接私有
    ...
    //定义了一系列的消息what，为什么会从1000开始？害怕冲突？？
    ...
    //将事件分发到view树,在view级别上，是由viewroot进行事件的处理的
    ...
    //经常抛出的错误原来在此处，那么currentthrread应该是哪个线程？？
    void checkThread() {
    ...
    //这又是什么玩意,实现了iwindow.stub,这个是什么东西，我真的不懂，但是很重要，擦
    ...
    //surfaceHolder,这又是干什么，控制surface，处理surface的方方面面
