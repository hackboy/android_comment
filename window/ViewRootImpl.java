...
//该类就是一个桥梁，用于连接view显示和view管理
//该类是一个hanler，就是说会处理大量的和view有关的消息
//实现了viewparent，即该类也要进行好viewgroup相似的view管理功能
...
public final class ViewRootImpl extends Handler implements ViewParent,
    ...
    //iwindowsession是何种作用，window回话，和window进行交互通信
    ...
    int mSeq;
    //这个view是干什么的哎,view树的顶级view，view处理的顶级起点，很重要的说，也就是说该类通过持有view树
    //的顶级view从而实现对整个view树的管理，妙哉
    View mView;
    View mFocusedView;
    ...
    //windowsession是什么，窗口会话，和哪个窗口会话，草草
    ...
    //构建一个viewroot，其初始化了很多，传入context，说明和context有很大联系,看看它都干了社么
     ...
     * We have one child
     */
    //该方法在windowmanagerimpl中被调用，看看它做了什么，该view代表了什么，顶级view的设置，一般是那个decorview
    //该view是viewrootimpl的管理view
    ...
    //做了什么，root这又不是一个view，肯定是给mView进行操作
     ...
     * {@inheritDoc}
     */
    //原来如此啊，很多只是简单的发送消息，所有的view的requestlayout都会最终到这个方法
    ...
    //真长啊，不是一般的长啊，也算得上是核心方法了，马丹，requestlayout最终会来到这个地方
    //执行遍历，从view树顶至底的一个遍历执行
                ...
                if (contentInsetsChanged) {
                    if (mWidth > 0 && mHeight > 0 &&
                        ...
                        try {
                            ...
                            //开始绘制了，理解这个调用过程，会不断的向下进行调用，学习数据结构吧骚年，很重要额
                            mView.draw(canvas);
    ...
    //为什么，和view中的view没有关系，更多的是处理一种绘制的逻辑，提供了一个surface
    ...
    private void deliverGenericMotionEvent(MotionEvent event, boolean sendDone) {
        ...
        //把事件传递给视图，使用其dispatchgenericmotionevnent方法
        //会一层层的传递下去
    ...
    //这是干啥，原来如此，这就是为什么不能在其他线程执行ui操作的原因
    void checkThread() {
        if (mThread != Thread.currentThread()) {
