...
//android窗口的实现，其实很简单的，代表了一个窗口对象，主要用于呈现内容视图
public class PhoneWindow extends Window implements MenuBuilder.Callback {
    ...
    //定义了一个顶级视图对象，该对象还是很有用的，因为他是该view树的顶端，很多都需要和该对象进行交互
    //为什么都喜欢继承自framelayout呢
    //该视图为我们实现了我们常见的标题栏等
    ...
    protected ViewGroup generateLayout(DecorView decor) {
        ...
        //根据不同的要求导入不同的布局文件，系统自身的
        View in = mLayoutInflater.inflate(layoutResource, null);
        //将导入的视图添加到decor中，这些是我们可以看到的标题栏等，该视图其中也包含了我们常用的内容视图
        decor.addView(in, new ViewGroup.LayoutParams(MATCH_PARENT, MATCH_PARENT));
        //从导入的布局文件中寻找用于盛放我们自定义布局内容的容器
    ...
    //创建一个顶级视图，在mDecor不存在的情况下，这个不仅仅创建了一个decor，还创建了mContentParent
    //并根据我们的配置生成了一个默认的窗口
