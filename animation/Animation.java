...
public abstract class Animation implements Cloneable {
    ...
    /***
     *该animation何时开始。当该动画开始时间被设置为START_ON_FIRST_FRAME时，该
     * 动画会第一次被执行
     ...
     */
    //获取在特定时间点应该施加的转换，一般子类覆盖applyTransformation方法提供该转换
    //参数为动画所处的时间和一个转换
    //即：在该时间点，应该对该对象进行什么样的变化，一个时间参数，一个转换对象参数
    //这个是最关键的部分
    public boolean getTransformation(long currentTime, Transformation outTransformation) {
        ...
        if ((normalizedTime >= 0.0f || mFillBefore) && (normalizedTime <= 1.0f || mFillAfter)) {
            ...
            //在此处进行调用,我们使用该方法定义自己的动画，这个地方也是实现我们效果的地方
