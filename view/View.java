...
//android中的view不仅仅是显示一个视图而已，还拥有诸多的事件处理、用户交互处理等责任
//view管理系统和view呈现系统之间的桥梁viewrootimpl
//android的view系统毫无例外的使用了组合模式
//在view树中每个元素都是一个view，这些view中有些是可以管理其他view的viewgroup，而有些则是独立的无子节点的view
//view类相当于组合模式中的component
//viewgroup则是composite
//widget则是leaf
/*			compontent(view)
			/		\
		   /
		 ...
		 /			    \
					      \
leaf(button......)	composite(viewgroup)===>addview removeview findview(直接使用接口进行定义实现==》viewmanager,viewparent)
理解一种思路远比理解一个孤立的代码意义大得多
*/
public class View implements Drawable.Callback, KeyEvent.Callback, AccessibilityEventSource {
    ...
    /***
     * 该view依附的窗口数量，显然我不懂，为什么会有多个
     */
    int mWindowAttachCount;

    //该参数一般会被父view使用以确定该view所需的布局参数
    ...
    //xml文件中导入视图，这个系统帮我们处理了很多细节并生成一个视图对象
    public View(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    /***
     * 执行从xml中导入view并应用指定的style. 这个style是什么意思，该方法允许
     * 使用自己的默认样式,defstyle代表该view独有的属性attr，比如R.attr.buttonStyle
     */
    public View(Context context, AttributeSet attrs, int defStyle) {
        this(context);
        //defStyle允许我们提供自己的默认样式，一般用于子类需要自己特殊的属性
    ...
    */
    ...
    /***
     * 把触屏事件向下传递到目标视图，如果该视图是目标视图，传递给自己
     ...
     * @return True if the event was handled by the view, false otherwise.
     */
    //理解一种思想，在此处就是一种递归的传递，树形结构，我要学习数据结构等
    //对于叶子view来说没意义
    public boolean dispatchTouchEvent(MotionEvent event) {
    	//这个判断是什么,是否向下传递，判断内容为三个
    	//此处也明白了ontouchevent和ontouch的区别
    ...
    //keyevent的一个回调的默认实现,传递一个keycode和keyevent
    ...
    /***
     * 显示该view的context menu， It is not safe to hold on to the
    ...
    /***
     * 实现该方法以处理触屏事件，返回true代表该事件被处理，其有默认的实现
     *
     * @param event The motion event.
     * @return True if the event was handled, false otherwise.
     */
    ...
    public void setLayoutParams(ViewGroup.LayoutParams params) {
        ...
        //会在对view参数改变的情况下调用一次requestLayout
        requestLayout();
    }
    ...
    //将由dirty定义的区域标记为需要绘制. I
    //调用父view的invalidateChild方法
    //最终也会调用performtravas方法
    ...
    /***
     * 将由l,t,r,b定义的矩形区域标记问需要绘制，这些坐标是相对于该view的.
     ...
     */
    //手动把该view绘制到指定的画布上，一般我们通过ondraw方法来定制我们的需求
    //据说使用了模版方法，定义了draw方法，封装了算法，具体由其子类去实现
     ...
     */
    //layout也是一种模版方法模式，通常用于封装的方法被设定为final以保持算法结构的一致性
    ...
    /***
     * 为该view赋予一个尺寸和位置，从layout调用，一般是父view的工作，一般用来确定该view是否需要重新布局
     *
    ...
    /***
     * 设置给定drawable作为背景，或者移除背景
     * If the background has padding, this View's padding is set to the background's
    ...
    public View getRootView() {
        ...
        View parent = this;
        //在attachinfo中未定义时，一直向上查找
        //循环直至最顶端的view，返回该最顶端view
    ...
    public void getLocationInWindow(int[] location) {
        ...
        //循环直至到顶端view，从而找出该view相对于顶端view的距离即是在window的距离
     ...
     * tree.
     */
    //擦擦，该方法很操蛋啊，直接调用父view的requestLayout方法，从而向上调用到顶层view的requestlayout方法
    public void requestLayout() {
        ...
        mPrivateFlags |= FORCE_LAYOUT;
        //辗转调用到performtravas方法
        if (mParent != null && !mParent.isLayoutRequested()) {
        	//直接调用父view的该方法，由此可知会直接调用到顶级视图的该方法
        	//viewgroup也没有重写该方法说明确实我是对的
    ...
    /***
     * <p>
     * 调用该方法获得view的高度和宽度，父view提供相关的约束信息
     * 为什么这样呢？因为对于布局参数来说，有时不是确切的值，比如match_parent而是要依赖于父view的相关信息,即要受到父view的约束
     * 父view将从view中获取的布局参数（layout_params）以及自身的信息传递给子view，子view据此计算出其所需空间即可
     * 
     * </p>
     *
     * <p>
     * 真正的测量工作由onMeasure完成，该方法只是调用onMeasure,因此，子类应该重写onMeasure方法
     * 参数由父view提供,父view从子view的布局参数中获取布局参数，并包装为measurespec，从而传递给子view
     * measurespec中的size是父view的吧。。。。。。我也不太清纯，比如我给你一个measurespec（size=100，spec='match_parent'）
     * 那么子view可能解析出就是width=100.
     * </p>
     */
    //比如父view的布局参数为相对布局，其布局参数也为相对布局，如何确定，这是一个递归的过程
    //该方法为final，并且会被父view进行调用，反正很蛋疼，参数如何确定
    //参数的确定才是核心部分，一个固定的算法骨架
    public final void measure(int widthMeasureSpec, int heightMeasureSpec) {
        if ((mPrivateFlags & FORCE_LAYOUT) == FORCE_LAYOUT ||
            ...
            //我们可以通过onmeasure定制我们自己的measures规则
    ...
    //依然是final，参数为计算过的宽高，确定两个值的意思
    protected final void setMeasuredDimension(int measuredWidth, int measuredHeight) {
        mMeasuredWidth = measuredWidth;
        mMeasuredHeight = measuredHeight;
        //出现这么多标识,应该是标识该view已测量过，measuredwidth和measuredHeight已经确定
        mPrivateFlags |= MEASURED_DIMENSION_SET;
    }
    ...
    /***
     * 立即启动动画,已懂得动画原理，再结合view看具体的实现
     */
    public void startAnimation(Animation animation) {
    	//调用setanimation，设置animation的启动时间
        animation.setStartTime(Animation.START_ON_FIRST_FRAME);
        setAnimation(animation);
        //invalidate 更新视图状态，实现动画
        invalidate();
    }
    ...
    /***
     * MeasureSpec是对父view传给view的一种测量依据的包装
     * 一般包括大小和模式，模式包括3种
     * 好简单
     */
    ...
    /***
     * 当一个触屏事件被分发到该view时调用的回调接口，该方法会在该触屏事件给予这个视图前调用
     * Interface definition for a callback to be invoked when a touch event is
     * dispatched to this view. The callback will be invoked before the touch
     * event is given to the view.
     */
    //他的ontouch方法的返回值还是很重要的
    ...
    /***
     * 定义当一个view的contextmenu被执行时的回调函数接口
    ...
    static class AttachInfo {
        ...
        /***
         * invalidateinfo用来传递ivalidate消息给一个handler。该消息包括目标view及
         * dirty区域的坐标
        ...
        /***
         * view树的顶级视图,通过getrootview获得的视图对象
         */
        View mRootView;
     ...
     * instances of View.</p>
     */
    //不懂其意，性能优化考虑，滚动条的实现机制，如果没有滚动条，这些属性就会多余，从而造成性能方面的浪费
