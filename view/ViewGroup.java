...
//把对叶子节点的管理做成了接口，应该是更加合理的做法,实现了对子view的管理
public abstract class ViewGroup extends View implements ViewParent, ViewManager {
    ...
    //子view数组，如何赋值，很简单，如果是导入xml，直接递归的加view，否则手动添加
     ...
     * {@inheritDoc}
     */
    //分发触屏事件，viewgroup的实现，要用到onInterceptTouchEvent方法
    @Override
    ...
    /***
     * 实现该方法以拦截所有的触屏事件，这允许你在事件分发到子view前监控这些事件，
     * 且让你获得任何点的当前手势的控制权
     ...
     * {@inheritDoc}
     */
    //分发绘制，回调用drawchild方法，进行子view的绘制,一个递归的实现，竟不是final，好滴很
    @Override
     ...
     */
    //绘制子视图，其中考虑动画的执行，该方法还是很重要的,哎呀我擦
     ...
     */
    //果然重写了该方法，父类的太操蛋了
    //有时不得不说我的抽象思维能力不行，比如对于该方法，这完完全全就是一种
    //迭代的方式，可以一直查询到view树的最后一个子节点，因为是向下递归的
    //不得不佩服别人写的这些代码
    //递归的过程，从顶至下
    @Override
    ...
    //移除子视图，这是最终的实现，和addviewinternal相似
    ...
    /***
     * 重绘子view，该类是实现view结构的基础，不要主动调用或修改，final类型
     * 貌似是一个不断向上调用的过程
     */
    ...
    /***
     * 完成测量子view的复杂部分：计算要传递给特定子view的测量规格，该方法为一个子view计算出
     * 正确的高度或宽度尺寸
     * 目的是把从子view的布局参数得到的测量规格信息进行结合得到最佳结果
    ...
    /***
     * 子view用于告诉其父view他们想如何放置
     * 基础的布局参数仅仅描述子view想要多高和多宽：
     * 有fill_parent和wrap_content两个值，或者确定的值
     * 很多viewgroup的子类扩展了布局参数类以适用于不同的目的
     */
    public static class LayoutParams {
         ...
         *              parameters' values
         */
        //原来如此，通过该方法获取到了view的viewlayout属性，即layout_width和layout_height
