...
//通过一个activitythread学习android建立activty的整个过程
//该类何时何处被使用
...
/***
 *一个程序必然属于一个进程，而其主线程是最重要的，该线程由
 * activitythread类进行管理，该类负责界面，activity等很多重要的元素
 * 对应用程序进程的主线程进行管理，安排和执行activity，brocast
 * 和其他作为activity manager请求的操作，一个主线程所要完成的工作，更高级别的任务
 */
//该类的用处，以及用在何处
//程序的主线程，承载很多核心功能，包括activity等都运行在该进程之中，学习了该类，很多问题比如说为何
//view更新不能再非ui线程中等
//用来对主线程进行管理吧
//很是重要的一个类，何处何时进行实例化
public final class ActivityThread {
    ...
    //持有一个ipackmanager，进行包管理，必然涉及到
    static IPackageManager sPackageManager;
    //applicationThread，应用程序线程
    final ApplicationThread mAppThread = new ApplicationThread();
    //创建消息循环，我欲知该类在何时被实例化
    final Looper mLooper = Looper.myLooper();
    //进行消息处理
    final H mH = new H();
    //远程对象和activity记录，activityrecord是activity的记录形式
    ...
    //主线程的handler
    static Handler sMainThreadHandler;  // set once in 
)

    Bundle mCoreSettings = null;
    //activity记录，对activity进行了一些包装，用于传递，包括其状态，其窗口，其配置等等
    static final class ActivityClientRecord {
        //一个activityclientrecord所包含的信息
        //该ibinder是什么，有什么作用
        IBinder token;
        int ident;
        Intent intent;
    ...
    //如此啊，activitythread和applicationthread，继承自applicationthreadnative，并且是一个private类型的类
    //这个是什么作用呢
    //applicationthreadnative是一个抽象类，实现了ibinder类
    private class ApplicationThread extends ApplicationThreadNative {
        ...
        //该方法真正的去将一个receiver调用，该方法又是在何处调用呢
        ...
        //该方法会在avtivitymanagerservice里进行调用，创建一个服务（service）
    ...
    //进行消息处理，核心部分了吧？，消息何时发送等等，操蛋
    private class H extends Handler {
        //各种消息类型，涉及到activity的方方面面
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case LAUNCH_ACTIVITY: {
                    ...
                    //调用handlelanuchactivity进行activity的启动
    ...
    //静态方法，获取包管理器，也学习一下在client端如何 获取服务binder并转化为本地接口
    //依然是通过getservice获取到，然后转换为本地的接口进行使用
    ...
    //获取系统级别的context，看其实现方法，果然看不懂啊 ⊙﹏⊙
    ...
    //执行加载activity，被handlelanuchactivity调用，返回一个activity对象,这样看行么，东西太多了
    private Activity performLaunchActivity(ActivityClientRecord r, Intent customIntent) {
        ...
        try {
            ...
            if (activity != null) {
                ...
                //很简单的一个调用，也正是此处调用了activity的onCreate方法
    ...
    private void handleLaunchActivity(ActivityClientRecord r, Intent customIntent) {
        ...
        if (a != null) {
            ...
            //handleresumeactivity是做什么的，果然resume，但是creae在哪
    ...
    //处理广播接收器，可以看到，其是在主线程中处理receiver，因此要明白为什么不能在其中处理
    //该方法何时被调用，处理流程又是怎样的
    private void handleReceiver(ReceiverData data) {
        // If we are getting ready to gc after going to the background, well
        // we are back active so skip it.
        unscheduleGcIdler();
        //获取到brocadcastreceiver对象类名，从而启动它
    ...
    private void handleCreateService(CreateServiceData data) {
        ...
        try {
            ...
            //进行context的初始化
            context.init(packageInfo, null, this);
            //为什么要创建一个application，难道是第一次？？
            Application app = packageInfo.makeApplication(false, mInstrumentation);
            //呵呵
            context.setOuterContext(service);
            //原来在此处调用了service的attach方法
            service.attach(context, this, data.info.name, data.token, app,
                    ActivityManagerNative.getDefault());
            //看到了oncreate的调用，呵呵，把调用者当sb啊
            service.onCreate();
    ...
    private void handleBindService(BindServiceData data) {
        ...
        if (s != null) {
            try {
                ...
                try {
                    if (!data.rebind) {
                        //调用service的onbind方法，返回一个Ibinder对象
    ...
    final Bundle performPauseActivity(ActivityClientRecord r, boolean finished,
        ...
        try {
            ...
            // Now we are idle.
            r.activity.mCalled = false;
            //把任务交给了nstrumentation以便对程序的运行进行监控
            mInstrumentation.callActivityOnPause(r.activity);
    ...
    //收集定义的回调，以便进行循环调用，boolean activities参数
    ...
    private final void performConfigurationChanged(
        ...
        if (shouldChangeConfig) {
            ...
            if (activity != null) {
                if (!activity.mCalled) {
                    //需要调用super.onConfigurationChanged
                    throw new SuperNotCalledException(
   ...
   //内存过低的处理     
    ...
    //获取一个icontentprovider,name代表什么呢
    ...
    //final方法，用于获取一个provider？要好好看一看这个方法，很重要的说
    //用于在本地对远程ContentProvider对象进行操作的
    ...
    //安装一个provider，这个是。。。。。provider是很麻烦的
    ...
    //该函数的作用是什么，attach是否成为系统进程，一般都是false，傻傻看不懂
    ...
    //该类的入口函数，开启消息循环，实例化该类，并开启消息循环,入口函数的作用是什么，和c的main应该是一个意思
    //这个类何时调用
    public static void main(String[] args) {
        ...
        //创建主线程的handler并进入消息循环
        if (sMainThreadHandler == null) {
            sMainThreadHandler = new Handler();
        }
        //实例化该类，然后进行一系列的操作，该类进行了什么操作
        ActivityThread thread = new ActivityThread();
        //主要还是用attach进行一些初始化操作
        thread.attach(false);
