...
/*** {@hide} */
//对外不公开，我也不知道他是做什么的,学习binder的用法即底层原理
public abstract class ApplicationThreadNative extends Binder
     ...
     * a proxy if needed.
     */
    //把一个binder对象转换为一个application thread接口，如有必要生成一个代理----》如有必要
    //是什么情况出现的必要
    //这里就体现了接口的重要性
    static public IApplicationThread asInterface(IBinder obj) {
        ...
        //先查询本地有没有这个接口，没有的话生成远端的代理
        IApplicationThread in =
            (IApplicationThread)obj.queryLocalInterface(descriptor);
        if (in != null) {
            return in;
        }
        //看来这个applicationthreadproxy还是很重要的
...
//为什么要定义这样一个类呢，其中的大部分要调用该类中的方法
class ApplicationThreadProxy implements IApplicationThread {
    //需要一个ibinder对象，然后该类作为其代理进行使用吗？
