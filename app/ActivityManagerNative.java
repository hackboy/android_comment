...
//IActivityMananger接口对IInterface接口进行了扩展，为什么自己写呢，不怕麻烦
//binder--->interface   interface--->binder
public abstract class ActivityManagerNative extends Binder implements IActivityManager{
    //这个binder对象一般是通过servicemanager.getService()得到
    static public IActivityManager asInterface(IBinder obj) {
        ...
        //如果没有查询到本地接口，则返回一个代理对象，如何才能执行到这一步，懂了
        //这个时候调用的方法就是代理进行中间的传输了，而不是直接调用服务的方法
        return new ActivityManagerProxy(obj);
    }

    /***
     * 单例模式获取一个activitymanager额代理接口吧
     */
    static public IActivityManager getDefault() {
        //返回的是activity服务的本地接口，全局唯一
        return gDefault.get();
    }
    ...
    //这个才是重中之重，和远端服务进行通信，主要工作由后端服务进行处理,哇哈，代理模式的体现么
    //实现如何将本地的请求发送到服务端
    //实际的处理请求的地方，里面的方法都未被实现，需要我们实现，这些在activitymanagerservice中实现
    ...
    private static final Singleton<IActivityManager> gDefault = new Singleton<IActivityManager>() {
        protected IActivityManager create() {
            //获取一个IBinder对象，其实应该是activitymanagerservice的binder代理
            IBinder b = ServiceManager.getService("activity");
            if (false) {
                Log.v("ActivityManager", "default service binder = " + b);
            }
            //asinterface方法，转换为本地接口，看asinterface是如何实现的
            //如果queryLocalInterface为空，那么实例化一个代理对象就行
            IActivityManager am = asInterface(b);
            if (false) {
                Log.v("ActivityManager", "default service = " + am);
            }
            //返回这个IactivityManager单例对象
            return am;
        }
    };
}

//典型的代理模式，给我一个远端的对象，将其进行包装即可
//大部分是进行请求的传输，处理有远端对象进行，代理不进行实际的操作，而是进行一个中转
class ActivityManagerProxy implements IActivityManager{
   ...
   //启动一个activity，这个参数叫做多啊 
    public int startActivity(IApplicationThread caller, Intent intent,
        ...
        //向ams发送数据，命令为START_ACTIVITY_TRANSACTION，这就是和后台服务进行通信
        //传递命令加参数加响应，是一个同步的操作，果然使用binder进行进程通信
        //调用远程binder的transact方法
    ...
    //发送广播，开启内部如何实现，包括如何寻找对应的broadcastreceiver等
    //传了十个参数，oooo
    ...
    //该方法返回一个intentsender对象，呼呼哈哈，不懂懂
    ...
    //一个远程额对象，和activitymanagerservice进行交互
