...
//packageManager的具体实现，通过packagemanagerservice服务进行操作
//让我们对其一览无余，为什么不直接使用packagemanager而要多创建一个applicationpackagemanager呢
final class ApplicationPackageManager extends PackageManager {
    private static final String TAG = "ApplicationPackageManager";
    private final static boolean DEBUG = false;
    private final static boolean DEBUG_ICONS = false;

    //flags的含义,在packagemanager中定义
    @Override
    ...
    //context会为我们实例化该类，获取一个构造完成的packagemanager对象，啊哈，这个ipackagemanager，
    //去看一下他的实现吧，我就呵呵了，很常见的用法
    ...
    //持有一个IPackageManager,用来调用packagemanagerservice的功能,应该是拥有相同的调用接口，该类在何处进行了赋值？
    //在进行该对象的构造时进行了赋值
    //在构造函数中进行赋值========》732行
    //mpm才是真正的代理，这个类毛也不是，就是一个给我们的接口，我们自己也能写一个
