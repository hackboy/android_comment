 ...
 * {@hide}
 */
//该类不公开，一般供内部使用,让我们一览无余,主要是对androidmanifest.xml文件的解析
//或者说packagemanagerservice通过该类对一个apk文件进行解析，从而获取该apk的一切相关信息，进而构造一个package
public class PackageParser {
    ...
    //apk文件中的manifest文件名====》AndroidManifest.xml
    private static final String ANDROID_MANIFEST_FILENAME = "AndroidManifest.xml";

    /*** @hide */
    //新权限信息，包括名字，版本，文件版本，估计为了兼容
    ...
    //解析provider，额，虽然我还不能很随意的定义自己的provider
    ...
    //定义一个内部类，package，待我一览无余,是对包的一个定义，包括包名，各组件信息，权限信息等一切清单文件的内容
    //学习内部类的用法特别是适用时机
    ...
    //定义一个component类，具体是干什么的额，代表一个组件，比如activity。。。
    ...
    //provider，内部类，做什么用，当然是contentprovider
